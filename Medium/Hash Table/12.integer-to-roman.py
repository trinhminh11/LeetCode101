...
Note: 49 is not 1 (I) less of 50 (L) because the conversion is based
on decimal places
...
class Solution:
    def intToRoman(self, num: int) -> str:
        ...
        while num > 0:
            ...
            # >1000
            if curr_iter >= 3:
                res += hashmap[1000] * num
                break

            if x == 9:
                res += hashmap[log10 * 10] + hashmap[log10]
            elif 5 <= x < 9:
                res += hashmap[log10] * (x - 5) + hashmap[log10 * 5]
            elif x == 4:
                res += hashmap[log10 * 5] + hashmap[log10]
            elif x < 4:
                res += hashmap[log10] * x
